...
let wishListContainer = document.querySelector("#Wishlists");
let favouriteBtn = document.querySelectorAll(".favourite");
...
for (let index = 0; index < products.length; index++) {
  favouriteBtn[index].addEventListener("click", () => {
    ...
    wishListContainer.appendChild(divSetion);
    if (favouriteBtn[index].textContent === "❤️") {
      favouriteBtn[index].addEventListener("click", () => {
        wishListContainer.removeChild(divSetion);
        favouriteBtn[index].textContent = "🤍";
